...
def getZScore(ratios, plotOrNot):
    ...
    if plotOrNot:
        ...
        plt.legend(['Ratio z-score', 'Mean', '+0.5', '-0.5'])
...
def getMovingIndex(ratios, train_pct, w1, w2, plotOrNot):
    ...
    if plotOrNot:
        ...
        plt.axhline(0.5, color='red', linestyle='--')
        plt.axhline(-0.5, color='green', linestyle='--')
        plt.legend(loc='upper right', labels=['Rolling Ratio z-Score', 'Mean', '+0.5', '-0.5'])
        plt.show()
...
xb_file = '02-10y.csv'
df = pd.read_csv(xb_file)
#测试相关性和协整关系
_, pv_coint, _ = coint(df['220210.IB'], df['220205.IB'])
corr, pv_corr = pearsonr(df['220210.IB'], df['220205.IB'])
...
S1, S2, ratios = getRatios(df, '220210.IB', '220205.IB', 1)
...
Trade2Contract(df, '220210.IB', '220205.IB', buy, sell, 10)
print(PairsTrade(S1, S2, 5, 10))