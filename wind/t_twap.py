...
def get_twap(df, instrument):
    ...
    ps1 = df.groupby(pd.Grouper(freq=freq)).apply(wavg, 'last', "ts_lag")
...
for filename in filenames:
    ...
    for index in range(len(dfs)):
        vwap_df.append(get_twap(dfs[index], instruments[index]))
    ...
    dfOutput.to_csv(filename+'_'+freq+"_twap_all.csv")
