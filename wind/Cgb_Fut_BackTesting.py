...
class BackTestingSystem:
    ...
    def getTickPrice(self, ts_date, isWind):
        ...
        if(len(ts_df) > 0 and isWind):
            ...
            tickPrices[0] = self.unwindEnhance(ts_df, len(ts_df), 'bid')
            tickTimes[0] = ts_df.iloc[0]['ticktime']
        elif (len(ts_df) > 0):
            #tickPrices[0] = ts_df.iloc[0]['ask']
            tickPrices[0] = self.unwindEnhance(ts_df, len(ts_df), 'ask')
            tickTimes[0] = ts_df.iloc[0]['ticktime']
        else:
        ...
        if (len(tf_df) > 0 and isWind):
            ...
            tickPrices[1] = self.unwindEnhance(tf_df, len(tf_df), 'ask')
            tickTimes[1] = tf_df.iloc[0]['ticktime']
        elif (len(tf_df) > 0):
            #tickPrices[1] = tf_df.iloc[0]['bid']
            tickPrices[1] = self.unwindEnhance(tf_df, len(tf_df), 'bid')
            tickTimes[1] = tf_df.iloc[0]['ticktime']
        else:
        ...
        if (len(t_df) > 0 and isWind):
            ...
            tickPrices[2] = self.unwindEnhance(t_df, len(t_df), 'bid')
            tickTimes[2] = t_df.iloc[0]['ticktime']
        elif (len(t_df) > 0):
            #tickPrices[2] = t_df.iloc[0]['ask']
            tickPrices[2] = self.unwindEnhance(t_df, len(t_df), 'ask')
            tickTimes[2] = t_df.iloc[0]['ticktime']
        else:
    ...
    def unwindEnhance(self, df, num, side):
